...
def timed(label: str, timings: dict):
    """Context manager that records elapsed time under the given label."""
    class Timer:
        def __enter__(self):
...
def develop_one_old(darktable: Path, dng: Path, tiff: Path, config_root: Path) -> None:
...
def develop_one(darktable: Path, dng: Path, tiff: Path, config_dir: Path) -> None:
    """Develop one DNG using a pre-created (reused) config directory."""
    command = [
        str(darktable), str(dng), str(tiff),
        "--core",
...
def develop_one_rawpy(dng: Path, tiff: Path) -> None:
    """Develop DNG with rawpy + adaptive filmic-style tone mapping."""
    ...
    # --- Measure scene brightness (linear domain) ---
    luma = img @ np.array([0.2126, 0.7152, 0.0722], dtype=np.float32)
    median_luma = float(np.median(luma))

    # --- Adaptive parameters ---
    # Dark frames → stronger lift, bright frames → gentler treatment
    # median_luma is typically in the 0.01–0.25 range for these scans
    if median_luma < 0.04:
        # Quite dark
        midtone_lift = 0.22
        dynamic_range = 11.5
        ...
        # Already bright – protect highlights
        midtone_lift = 0.10
        dynamic_range = 12.5
        shoulder_power = 1.40
    ...
    # --- Filmic curve (same as before, but with adaptive values) ---
    log_img = np.log2(img) + dynamic_range
    log_img = np.clip(log_img / dynamic_range, 0.0, 1.0)

    x = log_img
    toe = np.power(x, toe_power)
    shoulder = 1.0 - np.power(1.0 - x, shoulder_power)
    t = np.clip((x - 0.2) / 0.6, 0.0, 1.0)
    filmic = (1.0 - t) * toe + t * shoulder

    # Adaptive midtone lift
    filmic = filmic + midtone_lift * (1.0 - filmic) * filmic * 4.0
    filmic = np.clip(filmic, 0.0, 1.0)

    # Gentle highlight desaturation
    luma_f = filmic @ np.array([0.2126, 0.7152, 0.0722], dtype=np.float32)
    sat_fade = np.clip(1.0 - (luma_f - 0.82) / 0.18, 0.35, 1.0)
    filmic = luma_f[:, :, None] + sat_fade[:, :, None] * (filmic - luma_f[:, :, None])

    out = np.clip(filmic * 65535.0 + 0.5, 0, 65535).astype(np.uint16)
    tifffile.imwrite(tiff, out)
...
def main() -> None:
    ...
    for batch_index, batch in enumerate(batches, start=1):
        ...
        # Create one private config directory per workder (reused across frames)
        num_workers = max(1, args.jobs)
        worker_configs = []
        for i in range(num_workers):
            cfg = configs / f"worker_{i:02d}"
            cfg.mkdir(parents=True, exist_ok=True)
            worker_configs.append(cfg)

        from itertools import cycle
        config_cycle = cycle(worker_configs)

        def convert(path: Path) -> Path:
            target = tiffs / f"{path.stem}.tif"
            #develop_one_old(args.darktable, path, target, configs)
            develop_one_rawpy(path, target)
            config_dir = next(config_cycle)
            #develop_one(args.darktable, path, target, config_dir)
            return target

        # --- A. Darktable development ---
        ...
        for stage, seconds in sorted(timings.items(), key=lambda x: -x[1]):
            pct = 100 * seconds / total if total else 0
            print(f"    {stage:20s}  {seconds:7.1f}s  ({pct:5.1f}%)")
